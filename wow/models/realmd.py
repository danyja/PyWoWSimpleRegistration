...
class Account(db.Model):
    ...
    joindate = db.Column(db.DateTime, nullable=False, default=datetime.now())  #utcnow()?
...
class RealmCharacters(db.Model):
    ...
    __bind_key__ = 'realmd'
    realmid = db.Column(db.Integer, db.ForeignKey('realmlist.id'), nullable=False, default=0)  # 关联到realm表
    acctid = db.Column(db.BigInteger, db.ForeignKey('account.id'), nullable=False)  # 关联到account表
...
class RealmList(db.Model):
    ...
    realmflags = db.Column(db.SmallInteger, nullable=False, default=2,
...
class Uptime(db.Model):
    ...
    def __repr__(self):
        return (f'<Uptime realm={self.realmid} '
